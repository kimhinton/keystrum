    ...
    >
      ...
      <div className="mb-2 sm:mb-3 flex items-center gap-2 sm:gap-3 text-[10px] sm:text-xs font-mono">
        ...
        {lastChord && (
          ...
          >
            {lastChord.name} <span className="font-normal hidden sm:inline">{lastChord.label}</span>
          </span>
        )}
        {strumPulse && (
          <span
              ...
              {row.map((key, colIdx) => {
                ...
                return (
                  <button
                    ...
                    className="relative flex flex-1 min-w-0 sm:aspect-square h-11 sm:h-auto sm:min-h-0 items-center justify-center rounded sm:rounded-md md:rounded-lg font-mono text-[9px] sm:text-xs md:text-sm font-semibold transition-[transform,box-shadow,background] duration-75"
                    style={{
                      background: isActive
                        ? isMuteCol
